...
```
1. 모델 준비
   └ 일반 모델 로드 → Scale Tool로 피험자 크기에 맞춤
2. 실험 데이터 입력
   └ 모션 캡처(.trc), 지면반력(.mot) 파일 준비
3. 운동학 분석
   └ Inverse Kinematics → 관절 각도 산출
4. 동역학 분석
   └ Inverse Dynamics → 관절 모멘트 산출
   └ Static Optimization / CMC → 근육력 산출
5. 결과 분석
   └ Plot Tool로 시계열 출력, 통계 분석은 외부 도구
```
