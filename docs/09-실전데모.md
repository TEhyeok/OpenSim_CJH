...
## 9.9 로컬 PC에서 GUI 스크린샷 자동 수집

본인의 Mac에 OpenSim GUI가 설치되어 있다면, 다음 한 줄로
9장에 들어갈 실제 GUI 화면 5장을 반자동 수집할 수 있다.

```bash
bash scripts/local_capture_macos.sh           # 가이드 모드(권장)
bash scripts/local_capture_macos.sh --auto    # AppleScript 자동 모드
```

자세한 절차·권한 설정·문제 해결은 [scripts/README-local-capture.md](../scripts/README-local-capture.md)
참고. Windows·Linux 안내는 요청이 있으면 추가 가능하다.
